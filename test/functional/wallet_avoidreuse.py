...
class AvoidReuseTest(BitcoinTestFramework):
    ...
    def test_fund_send_fund_senddirty(self):
        ...
        # getbalances should show no used, 5 btc trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 0.009})

        # node 1 should now have about 5 btc left (for both cases)
        assert_approx(self.nodes[1].getbalance(), 0.01, 0.01)
        assert_approx(self.nodes[1].getbalance(avoid_reuse=False), 0.009, 0.001)
    ...
    def test_fund_send_fund_send(self):
        ...
        assert_approx(self.nodes[1].getbalance(), 0.1, 0.001)
        assert_approx(self.nodes[1].getbalance(avoid_reuse=False), 0.1, 0.001)
        ...
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 0.02})
